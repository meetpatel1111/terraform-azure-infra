...
locals {
  # Use STRING keys for for_each (Terraform requires maps/sets of strings)
  ...
  # Build (vm_index x disk_index) pairs to create and attach data disks
  vm_disk_pairs = {
    for pair in flatten([
...
# Public IPs — one per VM (optional)
...
# Linux Virtual Machines — one per VM
